...
export default function Prescriptions({ prescriptions, completedAppointments, filters }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        form.post('/doctor/prescriptions', {
            ...
            onError: (errors) => {
                const errorList = Object.values(errors).flat().map((msg) => `<li class="text-sm">${msg}</li>`).join('');
                Swal.fire({
                    icon: 'error',
                    title: 'Failed',
                    html: `<ul class="list-disc pl-5 space-y-1 text-left text-red-600">${errorList}</ul>`,
                    confirmButtonColor: '#0787f7',
                });
            },
            preserveScroll: true,
                    ...
                    <div className="flex items-center gap-3">
                        <form onSubmit={handleSearch} className="relative">
                            ...
                            <input
                                ...
                                className="h-9 w-48 rounded-lg border border-neutral-200 bg-white pl-9 pr-3 text-sm placeholder:text-neutral-400 focus:border-[#0787f7] focus:ring-1 focus:ring-[#0787f7] focus:outline-none dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100"
                            />
                        </form>
                        <button
                            onClick={openCreate}
                            className="inline-flex items-center gap-2 rounded-lg bg-[#0787f7] px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-[#0670d4]"
                        >
                        ...
                        <table className="w-full text-left text-xs">
                            <thead className="border-b border-neutral-100 bg-neutral-50 dark:border-neutral-700 dark:bg-neutral-800">
                                <tr>
                                    ...
                                    <th className="px-3 py-2 font-semibold text-neutral-600 dark:text-neutral-300">Instructions</th>
                                    <th className="px-3 py-2 font-semibold text-neutral-600 dark:text-neutral-300">Follow-up</th>
                                    <th className="px-3 py-2 font-semibold text-neutral-600 dark:text-neutral-300">Created</th>
                                </tr>
                            </thead>
                                ...
                                {prescriptions.data.map((rx) => (
                                    <tr key={rx.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                                        ...
                                        <td className="whitespace-nowrap px-3 py-2 text-neutral-600 dark:text-neutral-300">
                                            {rx.appointment ? format(new Date(rx.appointment.date), 'MMM d, yyyy') : '—'}
                                        </td>
                                        ...
                                        <td className="max-w-[120px] truncate px-3 py-2 text-neutral-500">{rx.instructions || '—'}</td>
                                        <td className="whitespace-nowrap px-3 py-2 text-neutral-500">{rx.follow_up || '—'}</td>
                                        <td className="whitespace-nowrap px-3 py-2 text-neutral-400">{format(new Date(rx.created_at), 'MMM d, yyyy')}</td>
                                    </tr>
                                ))}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl dark:bg-neutral-900">
                        <div className="mb-5 flex items-center justify-between">
                            <h2 className="text-lg font-bold text-neutral-900 dark:text-neutral-100">New Prescription</h2>
                            ...
                            <div>
                                ...
                                {completedAppointments.length === 0 ? (
                                    <div className="py-6 text-center">
                                        <p className="text-sm text-neutral-400">No completed appointments without prescriptions.</p>
                                    </div>
                                ) : (
                                    <div className="max-h-[300px] space-y-2 overflow-y-auto">
                                        {completedAppointments.map((apt) => (
                                            <button
                                                ...
                                                className="flex w-full items-center gap-3 rounded-lg border border-neutral-200 p-3 text-left transition-colors hover:border-[#0787f7] hover:bg-[#0787f7]/5 dark:border-neutral-700"
                                            >
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-[10px] text-neutral-400">
                                                        {format(new Date(apt.date), 'MMM d, yyyy')} · {apt.session} · {apt.reason}
                                                    </p>
                                                </div>
                                            </button>
                                        ))}
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
                                <div className="rounded-lg border border-[#0787f7]/20 bg-[#0787f7]/5 p-3">
                                    ...
                                    <p className="text-[10px] text-neutral-500">
                                        {format(new Date(selectedAppointment.date), 'MMMM d, yyyy')} · {selectedAppointment.session} · {selectedAppointment.reason}
                                    </p>
                                </div>
